...
-w: 允许写模式
-i [file]: 解释一个r2脚本
-A: 在载入时分析可执行程序(交叉引用, 等等)
-n: Bare load. Do not load executable info as the entrypoint 
-c 'cmds': 运行r2执行命令(例如: r2 -wqc'wx 3c @ main')
-p [prj]: Creates a project for the file being analyzed (CC add a comment when opening a file as a project) 为正在分析的文件建立一个项目工程()
